...
namespace ElBruno.QwenTTS.VoiceCloning;
...
internal sealed class SpeechTokenizer : IDisposable
{
    ...
    public long[,,] EncodeFromWav(Stream stream)
    {
        var (samples, sampleRate) = MelSpectrogram.ReadWav(stream);
        if (sampleRate != SampleRate)
            samples = MelSpectrogram.Resample(samples, sampleRate, SampleRate);
